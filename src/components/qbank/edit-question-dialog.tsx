...
export function EditQuestionDialog({
  ...
  const [editedQuestion, setEditedQuestion] = React.useState<Question | null>(
    question
  );
  ...
  React.useEffect(() => {
    ...
    setSelectedImageFile(null); // Reset file on new dialog open
  }, [question]);
  ...
  const handleRemoveOption = (index: number) => {
    ...
    } else if (editedQuestion.questionType === 'checkbox' && Array.isArray(editedQuestion.correctAnswer)) {
  ...
  const handleCorrectCheckboxChange = (option: string, checked: boolean) => {
    let currentAnswers = (editedQuestion.correctAnswer as string[] || []);
    if (checked) {
        currentAnswers.push(option);
    } else {
        currentAnswers = currentAnswers.filter(ans => ans !== option);
    }
    handleFieldChange("correctAnswer", currentAnswers);
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let finalQuestion = { ...editedQuestion };

      // 1. Handle image upload if a new one is selected
      if (selectedImageFile) {
        try {
            const downloadURL = await uploadQuestionImage(editedQuestion.id, selectedImageFile);
            finalQuestion.imageUrl = downloadURL;
        } catch (uploadError) {
             console.error("[Client Upload Error]", uploadError);
             throw new Error("Image upload to storage failed. Please check the console for details.");
        }
      }

      // 2. Update the question in Firestore
      const result = await handleUpdateQuestion(finalQuestion);
    ...
    if (editedQuestion.questionType === 'mcq' && editedQuestion.options) {
        return (
            <div className="space-y-2">
                ...
                <RadioGroup value={editedQuestion.correctAnswer as string} onValueChange={(val) => handleFieldChange("correctAnswer", val)}>
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="max-w-3xl h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Make changes to the question details below and save.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
                ...
                >
                    ...
                    {imagePreview ? (
                        <Image src={imagePreview} alt="Question preview" width={200} height={150} className="mx-auto h-24 w-auto rounded-md object-contain" />
                    ) : (
                    ...
                    <div className="flex text-sm text-muted-foreground">
                        <span className="relative rounded-md font-medium text-primary focus-within:outline-none">
                            ...
                            <Input ref={imageInputRef} id="file-upload" name="file-upload" type="file" className="sr-only" onChange={handleImageFileChange} accept="image/*"/>
                        </span>
                        <p className="pl-1">{selectedImageFile ? `(${selectedImageFile.name})` : 'or drag and drop'}</p>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <div className="space-y-2">
                    ...
                    <Input id="subject" value={editedQuestion.subject} onChange={(e) => handleFieldChange("subject", e.target.value)} />
                </div>
                 <div className="space-y-2">
                    <Label htmlFor="chapter">Chapter</Label>
                    <Input id="chapter" value={editedQuestion.chapter} onChange={(e) => handleFieldChange("chapter", e.target.value)} />
                </div>
            </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <div className="space-y-2">
                    ...
                    <Select value={editedQuestion.questionType} onValueChange={(val) => {
                        if (val === 'checkbox') {
                            handleFieldChange("correctAnswer", Array.isArray(editedQuestion.correctAnswer) ? editedQuestion.correctAnswer : [editedQuestion.correctAnswer as string].filter(Boolean));
                        } else if (val === 'mcq') {
                            handleFieldChange("correctAnswer", Array.isArray(editedQuestion.correctAnswer) ? editedQuestion.correctAnswer[0] || "" : editedQuestion.correctAnswer);
                        }
                 ...
                 <div className="space-y-2">
                    <Label>Difficulty</Label>
                     <Select value={editedQuestion.difficulty} onValueChange={(val) => handleFieldChange("difficulty", val)}>
                        <SelectTrigger><SelectValue/></SelectTrigger>
                        <SelectContent>
            ...
            <div className="space-y-2">
                ...
                <Textarea id="explanation" value={editedQuestion.explanation || ""} onChange={(e) => handleFieldChange("explanation", e.target.value)} />
            </div>

...
import { Bookmark, BrainCircuit, CheckCircle2, CheckSquare, Circle, Copy, Edit, MoreVertical, Sparkles, Trash2, Wand2, XCircle, Loader2 } from "lucide-react";
...
export default function QuestionCard({ 
  ...
  // Effect to auto-submit checkbox answers in certain modes
  React.useEffect(() => {
    if (
  ...
  const categorizeAndSaveQuestion = async () => {
    ...
    try {
        // This flow now returns the updated question object without saving it.
        const categorizedQuestionData = await handleCategorizeQuestion(question);
        
        // Now, we explicitly save it.
        const result = await handleUpdateQuestion(categorizedQuestionData);

        if (result.success) {
            onUpdate(categorizedQuestionData); // Update the UI with the new data
            toast({
                title: "Categorized!",
  ...
  const showInstantResult = ((!isExamMode && isAnswered) || (isExamMode && !isExamFinished && examAnswerMode === 'during' && isAnswered));
  ...
  const getOptionClassName = (option: string) => {
    ...
    // Review mode (global show all, post-exam, or single card answered)
  ...
  const cleanQuestionText = question.questionText.replace(/^(question\s*\d*\s*[:.)-]?\s*)|(^\d+\s*[:.)-]?\s*)/i, '');
      ...
      <Card className="bg-card transition-colors duration-300 shadow-xl flex flex-col border-border">
        ...
        <CardContent className="flex-grow">
           ...
           {question.options && ['mcq', 'checkbox'].includes(question.questionType) && (
              <div className="space-y-2 mt-2">
                  {question.options.map((option, index) => (
                      <div 
                        key={index}
                        className={cn("p-3 rounded-md border flex items-center justify-between transition-all", getOptionClassName(option))}
           ...
           {showAllAnswers && !['mcq', 'checkbox'].includes(question.questionType) && (
              <div className="mt-4 p-3 rounded-md bg-green-500/20 border border-green-500">
                  ...
                  <p className="text-foreground">{Array.isArray(question.correctAnswer) ? question.correctAnswer.join(', ') : question.correctAnswer}</p>
              </div>
            )}
        </CardContent>
        <CardFooter className="flex flex-col sm:flex-row justify-between items-center mt-4 gap-4">
          ...
          <div className="flex flex-wrap items-center justify-center sm:justify-end gap-1">
            {!isLocked && (
                 <Button variant="card-outline" size="sm" className="px-2 text-xs sm:px-3 sm:text-sm" onClick={categorizeAndSaveQuestion} disabled={isCategorizing}>
                    {isCategorizing ? <Loader2 className="mr-1 sm:mr-2 h-4 w-4 animate-spin" /> : <Wand2 className="mr-1 sm:mr-2 h-4 w-4" />}
                    <span className="sm:inline">Categorize</span>
                </Button>
            )}
            <Button variant="card-outline" size="sm" className="px-2 text-xs sm:px-3 sm:text-sm" onClick={() => onExplainClick(question)}>
              <BrainCircuit className="mr-1 sm:mr-2 h-4 w-4"/>
              <span className="sm:inline">Explain</span>
            </Button>
            {!similarityScore && (
                <Button variant="card-outline" size="sm" className="px-2 text-xs sm:px-3 sm:text-sm" onClick={findSimilar} disabled={isFindingSimilar}>
